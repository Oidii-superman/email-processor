...
def upload_to_gcs(file_data, filename, mime_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'):
    ...
    try:
        ...
        safe_print(f"    ✅ Google Cloud Storageアップロード成功")
        
        return public_url
        
    except Exception as e:
        safe_print(f" GCSアップロードエラー: {e}")
        import traceback
        traceback.print_exc()
        return None
    ...
    for model_name in model_names:
        for attempt in range(max_retries):
            try:
                ...
                print(f" {model_name} JSONパースエラー: {e}")
...
def main():
    ...
    for i, email_data in enumerate(emails, 1):
        ...
        safe_print(f" 判定: {extracted.get('type')}")
        
        if extracted.get('type') == 'other':
            safe_print("  → その他メール（スキップ）")
            other_count += 1
            continue
        ...
        if success:
            ...
            if extracted.get('type') == 'engineer':
                engineer_count += 1
                if excel_skills:
                    safe_print(f"     Excelスキル: {len(excel_skills)}件")
                if file_url_str:
                    safe_print(f"     ファイルURL: {file_url_str}")
            else:
                project_count += 1
                if file_url_str:
                    safe_print(f"     ファイルURL: {file_url_str}")
